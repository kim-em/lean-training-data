...
## Writes a human readable summary of the tactics tested so far using `tactic_benchmark`.

SCRIPT_DIR=$( cd -- "$( dirname -- "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )

mkdir -p $SCRIPT_DIR/../out/tactic_benchmark
cd $SCRIPT_DIR/../out/tactic_benchmark
