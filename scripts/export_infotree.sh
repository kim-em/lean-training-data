...
# Results will go in `out/export_infotree/Mathlib.Logic.Hydra.json`.
...
if [ ${#ARGS[@]} -eq 0 ]; then
  ...
  if [ ! -f $DIR/$mod.json ]; then
    echo $mod
    if [ ! -f .lake/build/bin/export_infotree ]; then
      lake build export_infotree
    fi
    .lake/build/bin/export_infotree ${FLAGS[@]} $mod > $DIR/$mod._json && mv $DIR/$mod._json $DIR/$mod.json
  fi
fi
